"""
Multimodal LLM Extraction Test with False Negative Detection

Runs Multimodal LLM extraction tests on specified PDFs, detects false negatives,
and generates a comprehensive report with process flow documentation.

This test includes:
- Document Intelligence OCR (baseline)
- Scanned PDF detection
- Multimodal LLM fallback (with image rendering)
- Text-based LLM fallback (if multimodal fails)
- Line items extraction (new table structure)
- Higher-level invoice data capture
...
- test_multimodal_llm_extraction_standalone.py available
- enhanced_false_negative_detector.py available
"""
...
class MultimodalLLMExtractionTesterWithFalseNegativeDetection:
    """Runs Multimodal LLM extraction tests and detects false negatives"""
    
    def __init__(self):
        """Initialize Multimodal LLM extractor"""
    ...
    async def run_tests_on_pdfs(self, pdf_paths: List[str]) -> List[Dict[str, Any]]:
        """
        Run Multimodal LLM extraction tests on multiple PDFs.
        ...
        """
        self.log_process_step("INITIALIZATION", "Initializing Multimodal LLM extractor", "INFO")
        ...
        for pdf_path in pdf_paths:
            ...
            try:
                # Run Multimodal LLM extraction
                self.log_process_step("EXTRACTION", f"Running Multimodal LLM extraction for {pdf_name}", "INFO")
                result = await self.multimodal_extractor.extract_from_pdf(pdf_path)
                result["pdf_path"] = pdf_path
                result["pdf_name"] = pdf_name
                
                # Add line items data
                result["line_items_data"] = self._extract_line_items_data(result)
                
                all_results.append(result)
                self.log_process_step("EXTRACTION", f"Multimodal LLM extraction completed for {pdf_name}", "SUCCESS")
                
            except Exception as e:
                self.log_process_step("EXTRACTION", f"Multimodal LLM extraction failed for {pdf_name}: {e}", "ERROR")
                all_results.append({
                    "pdf_path": pdf_path,
                    "pdf_name": pdf_name,
                    "error": str(e),
    ...
    def _extract_line_items_data(self, result: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Extract line items data from extraction result"""
        line_items = []
        
        # Get invoice data from result
        invoice_data = result.get("extracted_fields", {})
        
        # Check if line_items field exists
        if "line_items" in invoice_data:
            line_items_value = invoice_data["line_items"].get("value", [])
            if isinstance(line_items_value, list):
                for idx, item in enumerate(line_items_value):
                    if isinstance(item, dict):
                        line_items.append({
                            ...
                            "pst_amount": item.get("pst_amount"),
                            "qst_amount": item.get("qst_amount"),
    ...
    def create_combined_dataframe(self, all_results: List[Dict[str, Any]], false_negatives_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        Includes both higher-level fields and line items.
        ...
        for result in all_results:
            ...
            # Add higher-level invoice fields
            for field_name, field_data in extracted_fields.items():
                ...
                rows.append({
                    ...
                    "confidence_category": self._categorize_confidence(field_data.get("confidence")),
                    "extracted_by_di": False,  # Multimodal uses DI as baseline
                    "extracted_by_multimodal": False,  # Could be enhanced by multimodal
                    "extracted_by_text_llm": False,  # Could be enhanced by text LLM
            ...
            # Add line items
            line_items_data = result.get("line_items_data", [])
            for item in line_items_data:
                rows.append({
                    ...
                    "extracted_by_di": False,  # Line items come from DI initially
                    "extracted_by_multimodal": False,  # Could be enhanced by multimodal
                    "extracted_by_text_llm": False,  # Could be enhanced by text LLM
    ...
    def generate_comprehensive_report(self, all_results: List[Dict[str, Any]], false_negatives_df: pd.DataFrame = None, output_path: str = None) -> str:
        ...
        """
        report = []
        report.append("# Multimodal LLM Extraction Test Report with False Negative Detection")
        report.append("")
        ...
        report.append("### 3. Scanned PDF Detection")
        report.append("- **Step:** Determine if PDF is scanned (image-based)")
        report.append("- **Description:** Analyze PDF structure to detect if it's a scanned document")
        report.append("- **Status:** ✅ Executed in test")
        report.append("- **Output:** Boolean flag indicating if PDF is scanned")
        report.append("")
        
        report.append("### 4. Field Extraction")
        ...
        report.append("### 5. Confidence Calculation")
        ...
        report.append("### 6. Low-Confidence Field Identification")
        report.append("- **Step:** Identify fields requiring LLM enhancement")
        report.append("- **Description:** Find fields with confidence < threshold or missing values")
        report.append("- **Status:** ✅ Executed in test")
        report.append("- **Output:** List of low-confidence fields for LLM processing")
        report.append("")
        
        report.append("### 7. Multimodal LLM Fallback (if scanned)")
        report.append("- **Step:** Multimodal LLM extraction with image rendering")
        report.append("- **Description:** Use Azure OpenAI Vision API to extract/improve fields from PDF images")
        report.append("- **Status:** ✅ Executed in test (if PDF is scanned, real Azure OpenAI Vision API)")
        report.append("- **Output:** Enhanced field values with improved confidence")
        report.append("- **Image Rendering:** PDF pages converted to base64 images for Vision API")
        report.append("")
        
        report.append("### 8. Text-Based LLM Fallback (if multimodal fails or not scanned)")
        report.append("- **Step:** Text-based LLM extraction")
        report.append("- **Description:** Use Azure OpenAI to extract/improve fields from DI text")
        report.append("- **Status:** ✅ Executed in test (if needed, real Azure OpenAI API)")
        report.append("- **Output:** Enhanced field values with improved confidence")
        report.append("")
        
        report.append("### 9. Line Items Processing")
        report.append("- **Step:** Extract and structure line items")
        report.append("- **Description:** Process line items from DI, structure for new table format")
        report.append("- **Status:** ✅ Executed in test")
        report.append("- **Output:** Line items array ready for line_items table")
        report.append("- **Structure:** Includes line_number, description, quantity, unit_price, amount, taxes, etc.")
        report.append("")
        
        report.append("### 10. Extraction Detection")
        ...
        report.append("### 11. False Negative Detection")
        ...
        for result in all_results:
            ...
            total_line_items += len(result.get("line_items_data", []))
        ...
        report.append(f"- **Scanned PDFs:** {scanned_count}")
        ...
        for result in all_results:
            ...
            coverage = result.get("canonical_fields_coverage", {})
            report.append(f"- **Is Scanned:** {'✅ Yes' if result.get('is_scanned') else '❌ No'}")
            ...
            report.append(f"- **Line Items:** {len(result.get('line_items_data', []))}")
            report.append(f"- **Overall Confidence:** {result.get('extraction_confidence', 0):.3f}")
            
            # Multimodal LLM statistics
            if result.get("multimodal_llm_triggered"):
                report.append(f"- **Multimodal LLM Triggered:** ✅ Yes")
                report.append(f"- **Multimodal LLM Success:** {'✅' if result.get('multimodal_llm_success') else '❌'}")
            else:
                report.append(f"- **Multimodal LLM Triggered:** ❌ No")
            
            # Text LLM statistics
            if result.get("text_llm_triggered"):
                report.append(f"- **Text LLM Triggered:** ✅ Yes")
                report.append(f"- **Text LLM Success:** {'✅' if result.get('text_llm_success') else '❌'}")
            else:
                report.append(f"- **Text LLM Triggered:** ❌ No")
        ...
        total_line_items = sum(len(r.get("line_items_data", [])) for r in all_results if "error" not in r)
        report.append(f"**Total Line Items Extracted:** {total_line_items}")
        report.append("")
...
async def main():
    ...
    print(f"\n{'='*100}")
    print(f"MULTIMODAL LLM EXTRACTION TEST WITH FALSE NEGATIVE DETECTION")
    print(f"{'='*100}")
    ...
    try:
        # Initialize tester
        tester = MultimodalLLMExtractionTesterWithFalseNegativeDetection()
        ...
        for result in all_results:
            ...
            for field_name, field_data in extracted_fields.items():
                ...
                field_rows.append({
                    ...
                    "confidence": field_data.get("confidence"),
                    "confidence_category": tester._categorize_confidence(field_data.get("confidence"))
                })
        
        field_df = pd.DataFrame(field_rows)
        temp_csv = "multimodal_llm_extraction_field_details_temp.csv"
        ...
        generate_detection_report(false_negatives_df, false_positives_df, potential_false_negatives_df, output_path='multimodal_llm_false_negatives_report.csv')
        ...
        output_csv = "multimodal_llm_extraction_with_false_negatives.csv"
        ...
        report_path = "multimodal_llm_extraction_test_report_with_false_negatives.md"
        ...
        print(f"  - {output_csv}")
        print(f"  - {report_path}")
        print(f"  - multimodal_llm_false_negatives_report.csv (comprehensive detection report)")

"""
Base LLM Extraction Test with False Negative Detection

Runs Base LLM extraction tests on specified PDFs, detects false negatives,
and generates a comprehensive report with process flow documentation.

This test includes:
- Document Intelligence OCR (baseline)
- Text-based LLM fallback for low-confidence fields
...
- test_llm_extraction_standalone.py available
...
class LLMExtractionTesterWithFalseNegativeDetection:
    """Runs Base LLM extraction tests and detects false negatives"""
    
    def __init__(self):
        """Initialize Base LLM extractor"""
    ...
    async def run_tests_on_pdfs(self, pdf_paths: List[str]) -> List[Dict[str, Any]]:
        """
        Run Base LLM extraction tests on multiple PDFs.
        ...
        """
        self.log_process_step("INITIALIZATION", "Initializing Base LLM extractor", "INFO")
        ...
        for pdf_path in pdf_paths:
            ...
            try:
                # Run Base LLM extraction
                self.log_process_step("EXTRACTION", f"Running Base LLM extraction for {pdf_name}", "INFO")
                result = await self.llm_extractor.extract_from_pdf(pdf_path)
                ...
                self.log_process_step("EXTRACTION", f"Base LLM extraction completed for {pdf_name}", "SUCCESS")
                
            except Exception as e:
                self.log_process_step("EXTRACTION", f"Base LLM extraction failed for {pdf_name}: {e}", "ERROR")
                all_results.append({
                    "pdf_path": pdf_path,
                    "pdf_name": pdf_name,
                    "error": str(e),
        ...
        for result in all_results:
            ...
            for item in line_items_data:
                rows.append({
                    ...
                    "extracted_by_llm": False,  # Could be enhanced by LLM
    ...
    def generate_comprehensive_report(self, all_results: List[Dict[str, Any]], false_negatives_df: pd.DataFrame = None, output_path: str = None) -> str:
        ...
        """
        report = []
        report.append("# Base LLM Extraction Test Report with False Negative Detection")
        report.append("")
        ...
        report.append("### 5. Low-Confidence Field Identification")
        ...
        report.append("### 6. Base LLM Fallback")
        report.append("- **Step:** Text-based LLM extraction")
        report.append("- **Description:** Use Azure OpenAI to extract/improve low-confidence fields from DI text")
        report.append("- **Status:** ✅ Executed in test (real Azure OpenAI API)")
        report.append("- **Output:** Enhanced field values with improved confidence")
        report.append("")
        
        report.append("### 7. Line Items Processing")
        ...
        report.append("### 8. Extraction Detection")
        ...
        report.append("### 9. False Negative Detection")
        ...
        report.append("- ❌ **Multimodal LLM:** Not executed (Base LLM only test)")
        ...
        for result in all_results:
            ...
            report.append(f"- **DI Extracted:** {coverage.get('di_extracted', 0)}")
            report.append(f"- **LLM Extracted:** {coverage.get('llm_extracted', 0)}")
            report.append(f"- **LLM Only:** {coverage.get('llm_only', 0)}")
            ...
            # LLM statistics
            if result.get("llm_triggered"):
                report.append(f"- **LLM Triggered:** ✅ Yes")
                report.append(f"- **LLM Success:** {'✅' if result.get('llm_success') else '❌'}")
                report.append(f"- **LLM Groups Processed:** {result.get('llm_groups_processed', 0)}")
                report.append(f"- **LLM Groups Succeeded:** {result.get('llm_groups_succeeded', 0)}")
                report.append(f"- **LLM Improved Fields:** {len(result.get('llm_improved_fields', []))}")
            else:
                report.append(f"- **LLM Triggered:** ❌ No")
...
async def main():
    ...
    print(f"\n{'='*100}")
    print(f"BASE LLM EXTRACTION TEST WITH FALSE NEGATIVE DETECTION")
    print(f"{'='*100}")
    ...
    try:
        # Initialize tester
        tester = LLMExtractionTesterWithFalseNegativeDetection()
        ...
        temp_csv = "llm_extraction_field_details_temp.csv"
        ...
        generate_detection_report(false_negatives_df, false_positives_df, potential_false_negatives_df, output_path='llm_false_negatives_report.csv')
        ...
        output_csv = "llm_extraction_with_false_negatives.csv"
        ...
        report_path = "llm_extraction_test_report_with_false_negatives.md"
        ...
        print(f"  - {output_csv}")
        print(f"  - {report_path}")
        print(f"  - llm_false_negatives_report.csv (comprehensive detection report)")

"""
Test real multimodal LLM extraction on a specific PDF document.
...
async def test_real_multimodal_llm_extraction():
    """Test real multimodal LLM extraction on the specified PDF"""
    print("\n" + "="*80)
    print("REAL MULTIMODAL LLM EXTRACTION TEST")
    print("="*80)
    ...
    if not multimodal_deployment:
        ...
        print(f"  AOAI_MULTIMODAL_DEPLOYMENT_NAME: {'Set' if settings.AOAI_MULTIMODAL_DEPLOYMENT_NAME else 'NOT SET'}")
    ...
    try:
        ...
        # Extract invoice
        print(f"\n[PROGRESS] Extracting invoice with DI and Multimodal LLM...")
        print(f"[INFO] This may take 60-90 seconds...")
        
        invoice_id = f"test-multimodal-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        print("\n" + "="*80)
        print("[OK] MULTIMODAL LLM EXTRACTION TEST COMPLETE")
        print("="*80)
...
if __name__ == "__main__":
    result = asyncio.run(test_real_multimodal_llm_extraction())
    if result:
